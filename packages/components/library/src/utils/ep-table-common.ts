...
export const getEpTableCommonEvents = (target: any) => {
  return {
    select: target.onSelect,
    selectAll: target.onSelectAll,
    selectionChange: target.onSelectionChange,
    rowClick: target.onRowClick,
    rowDblclick: target.onRowDblclick,
    sortChange: target.onSortChange,
    currentChange: target.onCurrentChange,
  }
}
